...
import { CharacterSwitcher, CharacterDropdown } from './components/Characters'
...
// ── Explore sayfası (basit) ───────────────────────────────────────
const Explore = () => {
  const [chars, setChars] = useState([])
  useEffect(() => {
    import('./lib/supabase').then(({ supabase }) =>
      supabase.from('characters').select('*, players(username)').order('created_at', { ascending: false }).limit(30).then(({ data }) => setChars(data || []))
    )
  }, [])
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: 14 }}>
        {chars.map(c => (
          <div key={c.id} style={{ background: 'var(--bg-card)', border: '1.5px solid var(--border-soft)', borderRadius: 16, padding: '20px 14px', textAlign: 'center', boxShadow: 'var(--shadow-sm)', cursor: 'pointer', transition: 'all .2s' }}
...
const Shell = () => {
  ...
  const NAV = [
    ...
    { id: 'dm',      icon: '✉️', label: 'Pulse Messages' },
    ...
    <div style={{ minHeight: '100vh' }}>
      ...
      <header style={{ position: 'sticky', top: 0, zIndex: 100, background: 'var(--bg-card)', borderBottom: '1.5px solid var(--border-soft)', backdropFilter: 'blur(12px)', WebkitBackdropFilter: 'blur(12px)' }}>
        <div style={{ maxWidth: 1140, margin: '0 auto', padding: '0 16px', height: 58, display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <button onClick={() => setTab('feed')} style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}>
            ...
            <span style={{ fontSize: 17, fontWeight: 800, color: 'var(--text-primary)', letterSpacing: -0.5, display: 'none', '@media(minWidth:480px)': { display: 'block' } }}>Pulse</span>
          </button>

          {/* Nav (desktop) */}
          <nav style={{ display: 'flex', gap: 2, flex: 1, justifyContent: 'center' }}>
            {NAV.map(n => (
              <button key={n.id} onClick={() => setTab(n.id)} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '7px 14px', borderRadius: 'var(--radius-full)', border: 'none', background: tab===n.id ? 'var(--accent-soft)' : 'transparent', color: tab===n.id ? 'var(--accent-text)' : 'var(--text-muted)', fontWeight: tab===n.id ? 700 : 500, fontSize: 13, cursor: 'pointer', transition: 'all .15s', fontFamily: 'var(--font-ui)' }}
          ...
          <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            ...
            <CharacterDropdown />
            <button onClick={() => setCharScreen(true)} title="Karakter değiştir" style={{ background: 'none', border: '1.5px solid var(--border-soft)', borderRadius: 'var(--radius-full)', padding: '6px 10px', cursor: 'pointer', fontSize: 13, color: 'var(--text-muted)', transition: 'all .2s', fontFamily: 'var(--font-ui)' }}
              onMouseEnter={e => e.currentTarget.style.borderColor='var(--accent)'}
              onMouseLeave={e => e.currentTarget.style.borderColor='var(--border-soft)'}>
              🫀
            </button>
      ...
      <nav style={{ display: window.innerWidth > 768 ? 'none' : 'flex', position: 'fixed', bottom: 0, left: 0, right: 0, background: 'var(--bg-card)', borderTop: '1.5px solid var(--border-soft)', padding: '8px 0', justifyContent: 'space-around', zIndex: 90 }}>
        {[...NAV, { id: 'char', icon: '🫀', label: 'Karakterim' }].map(n => (
          <button key={n.id} onClick={() => n.id==='char' ? setCharScreen(true) : setTab(n.id)} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3, background: 'none', border: 'none', color: (tab===n.id && n.id!=='char') ? 'var(--accent)' : 'var(--text-muted)', fontSize: 20, cursor: 'pointer', padding: '4px 12px', fontFamily: 'var(--font-ui)' }}>
            {n.icon}
